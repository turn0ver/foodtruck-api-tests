...
describe('GET /searchByName', () => {
  it('should return status code 200 if food trucks matching the applicant', async () => {
    const response = await axios.get<FoodTruck[]>(`${BASE_URL}/searchByName`, {
      params: { name: 'The Geez Freeze' },
    });

    expect(response.status).toBe(200);
    expect(response.data.length).toBe(1);
    response.data.forEach(truck => {
      expect(truck.applicant).toMatch(/The Geez Freeze/i);
  ...
  it('should return status code 405 when use method not allowed', async () => {
    const response = await axios.post(`${BASE_URL}/searchByName`, { name: 'The Geez Freeze' }, {
      validateStatus: () => true
    });
  ...
  it('should return status code 400 when is not using the required parameter', async () => {
    ...
    expect(errorData.errors['name']).toContain('The name field is required.');
  });
});
